...
std::string vignette = "";	 // 单色 16 位或 8 位图像，包含晕影作为像素衰减因子。有关示例，请参阅 TUM monoVO 数据集。
std::string gammaCalib = ""; // 伽马校准文件，包含单行 256 个值，将 [0..255] 映射到相应的辐照度值，即包含离散化逆响应函数
std::string source = "";	 // 是包含图像的文件夹或 .zip 存档
std::string calib = "";		 // 几何相机标定文件
double rescale = 1;
bool reverse = false;
...
void exitThread()
{
	...
	firstRosSpin = true;
	while (true) // 在这里，该循环会一直等待，直到接收到 SIGINT 信号就执行my_exit_handler
		pause();
}
