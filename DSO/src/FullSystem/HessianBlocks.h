...
#define SCALE_B_INVERSE (1.0f / SCALE_B)
	...
	// 帧和帧之间的相对位姿变换需要有一些预计算的步骤，定义在了struct FrameFramePrecalc中。
	...
	{
		EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
		EFFrame *efFrame; //!< 帧的能量函数,指向后端优化

		// constant info & pre-calculated values
		// DepthImageWrap* frame;
		FrameShell *shell; //!< 帧的"壳", 定义于FrameShell.h，保存帧的位姿信息如时间戳、相机到世界的SE3表示等

		//* 图像导数[0]:辐照度  [1]:x方向导数  [2]:y方向导数, （指针表示图像）
		Eigen::Vector3f *dI;			   //!< 原始图像的图像导数  // trace, fine tracking. Used for direction select (not for gradient histograms etc.)
		...
		//* 都是ID
		int frameID;				//!< 增加关键帧的序号(FrameShell)	// incremental ID for keyframes only!
		static int instanceCounter; //!< 计数器
		int idx;					//!< 激活关键帧的序号(FrameHessian)

		// Photometric Calibration Stuff
		float frameEnergyTH; //!< 设定的帧的能量函数的阈值 // set dynamically depending on tracking residual
		float ab_exposure;	 //!< 图像的曝光时间

		bool flaggedForMarginalization; //!< 标记该帧是否将要被边缘化

		std::vector<PointHessian *> pointHessians;			   //!< 所有活跃点的信息。所谓活跃点，是指它们在相机的视野中+其残差项仍在参与优化部分的计算contains all ACTIVE points.
		std::vector<PointHessian *> pointHessiansMarginalized; //!< 已经边缘化的地图点(随着系统的运行，滑动窗口需要不断添加新的帧，移除老的帧，在移除老的帧的时候需要 进行边缘化，即把删掉的关键帧中的 PointHessian 转成先验的地图点，即 pointHessiansMarginal- ized(已经边缘化的地图点))contains all MARGINALIZED points (= fully marginalized, usually because point went OOB.)
		std::vector<PointHessian *> pointHessiansOut;		   //!< 判断为外点的地图点(如果由于遮挡等导致 PointHessian 无法完成投影，那么他就属于外点)contains all OUTLIER points (= discarded.).
		std::vector<ImmaturePoint *> immaturePoints;		   //!< 未成熟的地图点contains all OUTLIER points (= discarded.).
		...
		// variable info.
		SE3 worldToCam_evalPT; //!< 在估计的相机位姿(在估计的相机位姿，注意位姿是增量变化的，而光度参数通过FEJ固定在初次处理中)
		...
		//* 其中包含了位姿的更新PRE_worldToCam = SE3::exp(w2c_leftEps()) * get_worldToCam_evalPT();
	...
	struct CalibHessian
	{
		...
		inline CalibHessian()
		{
			VecC initial_value = VecC::Zero(); // 相机的内参，包含fx,fy,cx,cy
			//* 初始化内参
	...
	struct PointHessian
	{
		...
		// static values
		float color[MAX_RES_PER_POINT];	  // 主导帧host frame中的颜色信息 colors in host frame
		float weights[MAX_RES_PER_POINT]; // 残差的权重，host-weights for respective residuals.

		float u, v;			//!< 像素点的位置
		int idx;			//!<
		float energyTH;		//!< 光度误差阈值
		FrameHessian *host; //!< 某一个点对应的主导帧host frame
		...
		//* 点的残差值
		// 为了构建优化问题，每个关键帧中的 PointHessian 可以往其他帧投影，投影会形成残差 项，
		// 放在 residuals 中，所有残差项相加即构成损失函数，可以进行优化求解
