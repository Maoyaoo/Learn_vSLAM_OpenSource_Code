...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double td)
{
    ...
    ROS_DEBUG("num of feature: %d", getFeatureCount());// 能够作为路标点的数量
    double parallax_sum = 0;// 所有特征点视差总和
    int parallax_num = 0;
    last_track_num = 0;// 被跟踪的个数
    for (auto &id_pts : image)
    {
        //id_pts:map<feature_id,vector<pair<camera_id,[x,y,z,u,v,vx,vy]>>>
        FeaturePerFrame f_per_fra(id_pts.second[0].second, td);

        int feature_id = id_pts.first;// 获得特征点ID

         // 1.1迭代器寻找feature list中是否已经存在这该特征点ID
        auto it = find_if(feature.begin(), feature.end(), [feature_id](const FeaturePerId &it)
                          { return it.feature_id == feature_id; });

        if (it == feature.end())//feature list中没有该特征点ID
        {
            feature.push_back(FeaturePerId(feature_id, frame_count));//新建一个FeaturePerId，并且添加到feature list中
            feature.back().feature_per_frame.push_back(f_per_fra);//为这个FeaturePerId增加一个FeaturePerFrame
        }
        else if (it->feature_id == feature_id)//feature list中有该特征点ID
        {
            it->feature_per_frame.push_back(f_per_fra);//增加一个FeaturePerFrame
            last_track_num++;// 此帧有多少相同特征点被跟踪
        }
    }

    // 2. 该帧追踪点数小于20或者窗口内帧的数目小于2，是关键帧
