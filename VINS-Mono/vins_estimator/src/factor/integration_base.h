...
public:
    ...
    // 该函数向IntegrationBase中的数据寄存器存入每一个IMU量测的时间间隔dt、加速度计量测值acc和陀螺仪量测值gyr，
    // 并基于量测值对IMU预积分项进行一步递推
    void push_back(double dt, const Eigen::Vector3d &acc, const Eigen::Vector3d &gyr)
    {

        dt_buf.push_back(dt);    // 间隔时间
        acc_buf.push_back(acc);  // 加速度数据
        gyr_buf.push_back(gyr);  // 陀螺仪数据
        propagate(dt, acc, gyr); // 预积分
    }

    /**
     * @brief 状态重传播函数
     * 该函数在后端完成优化，给出加速度计和陀螺仪零偏新的最优估计值后，
     * 基于新的估计值对已有的IMU预积分进行重递推，函数入口参数为优化后的零偏值_linearized_ba和_linearized_bg。
     *
    ...
    /*
    double _dt //时间间隔
    const Eigen::Vector3d &_acc_0 //上次(t时刻)测量加速度
    const Eigen::Vector3d &_gyr_0 //上次(t时刻)测量角速度
    const Eigen::Vector3d &_acc_1 //本次(t+1时刻)测量加速度
    const Eigen::Vector3d &_gyr_1 //本次(t+1时刻)测量角速度
    const Eigen::Vector3d &delta_p //相对初始参考系的位移(t时刻)
    const Eigen::Quaterniond &delta_q //相对初始参考系的姿态(t时刻)
    const Eigen::Vector3d &delta_v //相对初始参考系的速度(t时刻)
    const Eigen::Vector3d &linearized_ba //相对初始参考系的加速度偏差(t时刻)
    const Eigen::Vector3d &linearized_bg //相对初始参考系的角速度偏差(t时刻)
    Eigen::Vector3d &result_delta_p //位置变化量计算结果(t+1时刻)
    Eigen::Quaterniond &result_delta_q  //姿态变化量计算结果(t+1时刻)
    Eigen::Vector3d &result_delta_v //速度变化量计算结果(t+1时刻)
    Eigen::Vector3d &result_linearized_ba //加速度偏差计算结果(t+1时刻)
    Eigen::Vector3d &result_linearized_bg //角速度偏差计算结果(t+1时刻)
    bool update_jacobian//Jacobian矩阵更新选项
    */
    // 该函数用于求解IMU的预积分状态递推和更新对应的Jacobian矩阵
    // 和论文中推导采用欧拉积分法不同，代码中实际采用的是中值积分法，且认为在整个[ k , k + 1 ] [k,k+1][k,k+1]过程中，加速度计和陀螺仪零偏保持不变
    ...
    // IMU预积分传播方程(时间间隔,线加速度,角速度)
    // 其作用是积分计算两个关键帧之间IMU测量的变化量：旋转delta_q 速度delta_v 位移delta_p
    ...
    Eigen::Vector3d linearized_ba, linearized_bg;         // k帧图像时刻对应的IMU测量值
