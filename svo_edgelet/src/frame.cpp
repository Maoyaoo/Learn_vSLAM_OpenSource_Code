...
namespace svo
{
  ...
  Frame::Frame(svo::AbstractCamera *cam, const cv::Mat &img, double timestamp) : id_(frame_counter_++),
  ...
  void Frame::checkKeyPoints(Feature *ftr)
  {
    ...
    if (ftr->px[0] >= cu && ftr->px[1] < cv)
    {
      if (key_pts_[2] == NULL)
        key_pts_[2] = ftr;
      else if ((ftr->px[0] - cu) * (ftr->px[1] - cv) < (key_pts_[2]->px[0] - cu) * (key_pts_[2]->px[1] - cv))
        key_pts_[2] = ftr;
    }
    if (ftr->px[0] < cu && ftr->px[1] < cv)
    {
    ...
    if (ftr->px[0] < cu && ftr->px[1] >= cv)
    {
      if (key_pts_[4] == NULL)
        key_pts_[4] = ftr;
      else if ((ftr->px[0] - cu) * (ftr->px[1] - cv) < (key_pts_[4]->px[0] - cu) * (key_pts_[4]->px[1] - cv))
        key_pts_[4] = ftr;
    }
  }
