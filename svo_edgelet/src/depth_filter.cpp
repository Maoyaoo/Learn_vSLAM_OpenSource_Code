...
namespace svo
{
  ...
  DepthFilter::DepthFilter(feature_detection::DetectorPtr feature_detector, feature_detection::DetectorPtr edge_detector, callback_t seed_converged_cb) : feature_detector_(feature_detector),
  ...
  void DepthFilter::initializeSeeds(FramePtr frame)
  {
    ...
    feature_detector_->setExistingFeatures(frame->fts_); // 当前帧上的这些已经有的特征点，占据住网格。

    // 在当前帧的5层金字塔上，每层头提取fast点，首先用3x3范围的非极大值抑制。
    // 然后，对剩下的点，全部都计算shiTomasi分数，有点像Harris角点里面的那个分数。
    // 再全部映射到第0层的网格上，每个网格只保留分数最大的，且大于阈值的那个点。
    feature_detector_->detect(frame.get(), frame->img_pyr_,
                              Config::triangMinCornerScore(), new_features);

    // 找边缘点的话，都只在第0层上面找。同样也是画网格，然后再每个网格中找canny线，
    // 然后对于网格中的在canny线上的点，计算它的梯度的模，保留模梯度最大的那个点，作为边缘点。
    // 梯度方向是二维的，就是这个点的右左下上梯度。程序里用了cv::Scharr结合cv::magnitude来快速算出所有点的横纵方向的梯度。
    ...
    std::for_each(new_features.begin(), new_features.end(), [&](Feature *ftr)
                  {
                    ...
                    for (auto it = frame->fts_.begin(), ite = frame->fts_.end(); it != ite; ++it)
                    {
                      if ((*it)->point != NULL)
                      {
                        Vector2d dist = (ftr->px - (*it)->px);//计算新特征点和原来跟踪的时候剩下特征点的最小距离
                        if (dist.norm() < dist_min)
                        {
                          dist_min = dist.norm();
                          z = frame->w2f((*it)->point->pos_).z();//两个特征点最近的
                        }
                      }
                    }
                    if (dist_min < 70)
  ...
  void DepthFilter::updateSeeds(FramePtr frame)
  {
    ...
    // 只更新有限数量的种子，因为我们没有时间在每一帧中更新所有的种子!
    ...
    while (it != seeds_.end())
    {
      ...
      */
      ...
      {
        it->b++; // increase outlier probability when no match was found//当没有找到匹配时，增加异常值概率
        ++it;
        ++n_failed_matches;
        continue;
      }
      ...
      // compute tau
      double tau = computeTau(T_ref_cur, it->ftr->f, z, px_error_angle); // 计算深度估计的误差
  ...
  // 该函数通过融合当前深度估计和先前的深度估计信息，更新深度滤波器中的种子的状态，以提高深度估计的精度和稳定性。
