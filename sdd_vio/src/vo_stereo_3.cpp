...
namespace sdd_vio
{
     ...
     *      lambda_标识点是否在视野中的掩码
     *      up_factor_LMA中lambda增长的因子。
     *      down_factor_LMA中lambda降低的因子。
     *，这个函数通过前向组合方法结合Levenberg-Marquardt算法，在视觉和惯性数据的协同作用下进行优化，
     * 以求得当前帧相对于关键帧的最佳变换，从而实现视觉-惯性融合定位。
     */
    void VoStereo::optimize_fca(const cv::Mat &img_curr, Eigen::Isometry3f &T_curr, Eigen::Isometry3f &T_wb, Eigen::Isometry3f &T, Eigen::Vector3f &v, int ilay)
    {
        ...
        //使用Sobel算子计算水平方向（Gx_mat）和垂直方向（Gy_mat）的梯度图像
        ...
        Eigen::MatrixXf J_masked; // every time J_masked is re-calculated for every pixel inside the view //被遮罩的雅可比矩阵，用于视野内的点
        Eigen::VectorXf error_masked;//误差向量，包含所有视野内点的误差
        Eigen::VectorXf W; // diagonal weight matrix for Huber estimation   Huber估计的对角权重矩阵

        /* Jacobian and error vector of IMU */
        //IMU的雅可比矩阵和误差向量：
        Eigen::MatrixXf J_imu;//IMU雅可比矩阵
        Eigen::VectorXf r_imu;//IMU误差向量
        Eigen::MatrixXf Z_imu; // inverse covariance of imu errors IMU误差的逆协方差矩阵
        J_imu.resize(9, 9);
        ...
        //设置初始的lambda、up_factor、down_factor，以及迭代的终止条件。
        /* Levenberg Marquardt */
        ...
        for (int iter = 0; iter < max_num_iter_; ++iter)
        {
            ...
            if (use_lma_ == false)
            {
                ...
                //如果误差增加、误差变化率小于目标变化率或达到最大迭代次数，则终止优化
