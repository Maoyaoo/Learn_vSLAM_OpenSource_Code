...
//调整和可视化一个 OpenCV 矩阵 (cv::Mat)，将其值缩放到 0 到 255 的范围，以便在图像显示时能更清楚地观察到其细节
cv::Mat adjustVis(const cv::Mat& M)
{
    double min, max;
    cv::minMaxIdx(M, &min, &max);//查找最小值和最大值
    cv::Mat M_shifted = M - min;//这行代码将整个矩阵的值平移，使得最小值变为 0
    cv::Mat adjM;
    cv::convertScaleAbs(M_shifted, adjM, 255 / (max-min));//缩放到 0-255 的范围
    return adjM;
}
