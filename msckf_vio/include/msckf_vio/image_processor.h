...
namespace msckf_vio
{
  ...
  public:
    ...
    typedef boost::shared_ptr<const ImageProcessor> ConstPtr; // ConstPtr 指向的是一个常量的 ImageProcessor 对象。
  ...
  private:
     ...
     * 接收双目图像，进行双目特征跟踪，先光流跟踪上一帧的特征点，然后提取新的特征点。
     * 将跟踪到的特征点publish出去，供后端接收使用。
     */
    void stereoCallback(
     ...
     * @param msg IMU msg.
     * 接收IMU数据，将IMU数据存到imu_msg_buffer中，这里并不处理数据
     */
    ...
    // cv_bridge::CvImageConstPtr 是一个指向 cv_bridge::CvImage 类型的常量智能指针
    /* cv_bridge::CvImage 本身是一个包含图像数据和相关信息的结构体，它的成员通常包括：
     * image: 一个 cv::Mat 对象，存储实际的图像数据。
     * encoding: 图像的编码格式（例如，bgr8, mono8 等）。
     * header: 包含图像的时间戳和坐标帧等元信息。
     */
