...
namespace msckf_vio
{
  ...
  void ImageProcessor::createImagePyramids()
  {
    ...
    // opencv自带的函数
    // I (InputArray)：输入的图像。它可以是单通道灰度图像（通常为 CV_8U 或 CV_32F 类型），用于计算光流的图像。
    //  pyramid (std::vector<Mat>&)：这个参数是一个输出参数，返回一个 Mat 类型的图像金字塔。在金字塔的每一层，图像的分辨率都会减小，分辨率从最底层（原始图像）开始逐层减少。
    //  winSize (Size)：光流算法中使用的窗口大小。这是用于计算每个像素的光流时所使用的局部区域的大小。通常情况下，较大的窗口会提供更平滑的估计，但可能丧失细节信息。
    //  maxLevel (int)：金字塔的最大层数。层数越多，图像的分辨率会逐渐降低，适用于处理大尺度的运动。金字塔的层数越多，计算的开销也会增大。
    //  withDerivatives (bool, 默认值：false)：是否计算图像梯度（导数）。如果设置为 true，在构建金字塔时，OpenCV 会计算图像的梯度（Ix 和 Iy）。这对某些计算光流的算法（例如 Horn-Schunck）非常有用。
    //  pyrBorder (int, 默认值：BORDER_REFLECT_101)：金字塔图像边界的处理方式，定义了在图像边界如何填充。常用的边界类型有 BORDER_CONSTANT、BORDER_REFLECT 等。
    //  tryReuseInputImage (bool, 默认值：false)：是否复用输入图像。如果设置为 true，OpenCV 会尝试复用输入图像（而不是每次都创建一个新的副本）。这样可能有助于节省内存和计算资源，但要确保输入图像在金字塔计算后不会被修改。
   ...
   * 左图特征点前后帧跟踪，得到当前帧左图特征点,去除外点
   * 当前帧左右图跟踪，得到当前帧右图特征点,进一步去除一些外点
   * 最后左右目分别跟上一帧左右目做ransac剔除外点
   */
      ...
      vector<unsigned char> &inlier_markers)
  {
    ...
    // Track features using LK optical flow method.
    // 3. 光流匹配找点，中间去除一些误匹配
    calcOpticalFlowPyrLK(curr_cam0_pyramid_, curr_cam1_pyramid_,                         // 左相机与右相机当前帧的图片（多层金字塔）
   ...
   * @param  pts_in 输入像素点
   * @param  intrinsics fx, fy, cx, cy 内参
      ...
      const cv::Vec4d &new_intrinsics)
  {
    ...
    {
      // rectification_matrix是左相机到右相机的旋转R_cam0_cam1
